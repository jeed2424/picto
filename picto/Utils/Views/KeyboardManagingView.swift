...
open class KeyboardManagingView: UIView {

    /// The Cancellables
    open var cancellables = CompositeCancellable()
    ...
    open func keyboardDidShow(_ size: CGSize) {
        if enableTapAnywhereToDismiss {
            addGestureRecognizer(keyboardDismissGestureRecognizer)
        }
    ...
    open func keyboardWillHide() {
        activeField = nil
        if enableTapAnywhereToDismiss {
            removeGestureRecognizer(keyboardDismissGestureRecognizer)
        }
        resetKeyboardOffset()
    }

    open func getFirstResponderForKeyboardManagement() -> UIView? {
        activeField ?? firstResponder()
    }
