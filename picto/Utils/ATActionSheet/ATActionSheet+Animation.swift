...
extension ATActionSheet {
    ...
    func hideController(completion: ((Bool) -> Void)? = nil) {
        UIView.setAnimationsEnabled(true)
        UIView.animate(withDuration: 1.0, delay: 0, usingSpringWithDamping: 0.85, initialSpringVelocity: 0.02, options: .curveLinear, animations: {
            self.alertView.transform = CGAffineTransform(translationX: 0, y: self.alertView.frame.height)
            self.backgroundView.alpha = 0 }) { bool in
            completion?(bool)
        }
    }
...
extension UIView {
    var globalPoint: CGPoint? {
        if let s = self.superview {
            return s.convert(self.frame.origin, to: nil)
        } else {
            return self.convert(self.frame.origin, to: nil)
        }
